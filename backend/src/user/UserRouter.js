...
router.patch('/api/v1/users/:token/active', async (req, res, next) => {
...
router.post('/api/v1/users/password-reset', check('email').isEmail().withMessage('email_invalid'), async (req, res, next) => {
  const errors = validationResult(req);
  if (!errors.isEmpty()) {
    return next(new ValidationException(errors.array()));
  }
...
router.patch(
  ...
  async (req, res, next) => {
    ...
    try {
      await UserService.updatePassword(req.params.token, req.body);
